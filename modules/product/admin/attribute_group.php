...
{
	$json = array();
	$attribute_group_id = $nv_Request->get_int( 'attribute_group_id', 'post', 0 );
	...
	elseif( $token == md5( $global_config['sitekey'] . session_id() . $attribute_group_id ) )
	{
		$del_array = array( $attribute_group_id );
	}
	...
	if( ! empty( $del_array ) )
	{
		...
		$_del_array = array();
		$no_del_array = array();
		foreach( $del_array as $attribute_group_id )
		{
 
			$attribute_total = $db->query( 'SELECT COUNT(*) total FROM ' . TABLE_PRODUCT_NAME . '_attribute WHERE attribute_group_id = ' . ( int )$attribute_group_id )->fetchColumn();

			if( $attribute_total )
			{
				$error['warning'] = sprintf( $lang_ext['error_attribute'], $attribute_total );
			}

			if( isset( $error['warning'] ) )
			{
				$no_del_array[] = $attribute_group_id;
			}
			else
			{
				$db->query( 'DELETE FROM ' . TABLE_PRODUCT_NAME . '_attribute_group WHERE attribute_group_id = ' . ( int )$attribute_group_id );
				$db->query( 'DELETE FROM ' . TABLE_PRODUCT_NAME . '_attribute_group_description WHERE attribute_group_id = ' . ( int )$attribute_group_id );

				$json['id'][$a] = $attribute_group_id;

				$_del_array[] = $attribute_group_id;
		...
		{
			nv_insert_logs( NV_LANG_DATA, $module_name, 'log_del_attribute_group', implode( ', ', $_del_array ), $admin_info['userid'] );
...
elseif( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	$data = array( 'attribute_group_id' => 0, 'sort_order' => 0 );

	foreach( $getLangModId as $language_id => $value )
	{
		$data['attribute_group_description'][$language_id] = array( 'name' => '' );
	}

	$data['attribute_group_id'] = $nv_Request->get_int( 'attribute_group_id', 'get,post', 0 );

	if( $data['attribute_group_id'] > 0 )
	{
		$data = $db->query( 'SELECT *
		FROM ' . TABLE_PRODUCT_NAME . '_attribute_group  
		WHERE attribute_group_id=' . $data['attribute_group_id'] )->fetch();

		$sql = 'SELECT * FROM ' . TABLE_PRODUCT_NAME . '_attribute_group_description WHERE attribute_group_id=' . $data['attribute_group_id'];
		...
		while( $rows = $result->fetch() )
		{
			...
			$data['attribute_group_description'][$rows['language_id']] = $rows;
		}
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		$data['attribute_group_description'] = $nv_Request->get_typed_array( 'attribute_group_description', 'post', array() );
		...
		if( empty( $error ) )
		{
			if( $data['attribute_group_id'] == 0 )
			{
				...
				$stmt = $db->prepare( 'INSERT INTO ' . TABLE_PRODUCT_NAME . '_attribute_group SET 
				sort_order = ' . ( int )$data['sort_order'] );
				$stmt->execute();

				if( $data['attribute_group_id'] = $db->lastInsertId() )
				{
					$stmt->closeCursor();
					...
					foreach( $data['attribute_group_description'] as $language_id => $value )
					{
						...
						try
						{
							$stmt = $db->prepare( 'INSERT INTO ' . TABLE_PRODUCT_NAME . '_attribute_group_description SET 
					...
					nv_insert_logs( NV_LANG_DATA, $module_name, 'Add Attribute group description', 'attribute_group_id: ' . $data['attribute_group_id'], $admin_info['userid'] );
			...
			else
			{
				try
				{
					$stmt = $db->prepare( 'UPDATE ' . TABLE_PRODUCT_NAME . '_attribute_group SET 
					sort_order = ' . ( int )$data['sort_order'] . ' 
					WHERE attribute_group_id = ' . ( int )$data['attribute_group_id'] );
					...
					if( $stmt->execute() )
					{
						...
						$db->query( 'DELETE FROM ' . TABLE_PRODUCT_NAME . '_attribute_group_description WHERE attribute_group_id = ' . ( int )$data['attribute_group_id'] );
						...
						nv_insert_logs( NV_LANG_DATA, $module_name, 'Edit Attribute group description', 'attribute_group_id: ' . $data['attribute_group_id'], $admin_info['userid'] );
		...
		{
			$nv_Cache->delMod( $module_name );
			Header( 'Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=attribute_group' );
			die();
		}

	}

	$xtpl = new XTemplate( 'attribute_group_add.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file . '/product' );
...
/*show list attribute group*/

$per_page = 50;
$page = $nv_Request->get_int( 'page', 'get', 1 );

$sql = TABLE_PRODUCT_NAME . '_attribute_group ag 
LEFT JOIN ' . TABLE_PRODUCT_NAME . '_attribute_group_description agd 
...
$xtpl = new XTemplate( 'attribute_group.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file . '/product' );
$xtpl->assign( 'LANG', $lang_module );
$xtpl->assign( 'LANGE', $lang_ext );
$xtpl->assign( 'AddMenu', AddMenu() );
$xtpl->assign( 'NV_LANG_VARIABLE', NV_LANG_VARIABLE );
...
if( ! empty( $dataContent ) )
{
	foreach( $dataContent as $item )
	{
		...
		$item['token'] = md5( $global_config['sitekey'] . session_id() . $item['attribute_group_id'] );

		$item['edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&action=edit&token=' . $item['token'] . '&attribute_group_id=' . $item['attribute_group_id'];
