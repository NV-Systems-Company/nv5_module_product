...
{
	$json = array();
	$stock_status_id = $nv_Request->get_int( 'stock_status_id', 'post', 0 );
	...
	elseif( $token == md5( $global_config['sitekey'] . session_id() . $stock_status_id ) )
	{
		$del_array = array( $stock_status_id );
	}

	if( ! empty( $del_array ) )
	{

		$sql = 'SELECT stock_status_id, name FROM ' . TABLE_PRODUCT_NAME . '_stock_status  
		WHERE language_id=' . $ProductGeneral->current_language_id . ' AND stock_status_id IN (' . implode( ',', $del_array ) . ')';
		$result = $db->query( $sql );
		$del_array = $no_del_array = array();
		$a = 0;
		while( list( $stock_status_id, $title ) = $result->fetch( 3 ) )
		{
			$product_total = $db->query( 'SELECT COUNT(*) total FROM ' . TABLE_PRODUCT_NAME . '_product WHERE stock_status_id = ' . intval( $stock_status_id ) )->fetchColumn();
			if( $product_total == 0 )
			{
				if( $db->query( 'DELETE FROM ' . TABLE_PRODUCT_NAME . '_stock_status WHERE stock_status_id = ' . intval( $stock_status_id ) ) )
				{

					$json['id'][$a] = $stock_status_id;
					$del_array[] = $stock_status_id;
					++$a;
				}
				else
				{
					$no_del_array[] = $stock_status_id;
				}
			}
			else
			{
		...
		if( $count )
		{
			nv_insert_logs( NV_LANG_DATA, $module_name, 'log_del_stock_status', implode( ', ', $del_array ), $admin_info['userid'] );

			$nv_Request->unset_request( $module_data . '_success', 'session' );

			$json['success'] = $lang_ext['text_delete_success'];

			$ProductGeneral->deleteCache('stock_status');
...
elseif( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	{
		$data['stock_status'][$language_id] = array( 'name' => '' );
	}
	$error = array();
	$error_key = array();

	$stock_status_id = $nv_Request->get_int( 'stock_status_id', 'get,post', 0 );
	if( $stock_status_id > 0 )
	{
		$data = $db->query( 'SELECT * FROM ' . TABLE_PRODUCT_NAME . '_stock_status WHERE stock_status_id=' . $stock_status_id )->fetchAll();

		foreach( $data as $value )
		{
			$data['stock_status'][$value['language_id']] = array( 'name' => $value['name'] );
		}
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		$data['stock_status'] = $nv_Request->get_typed_array( 'stock_status', 'post', array() );
		...
		if( empty( $error ) )
		{
			if( $data['stock_status_id'] == 0 )
			{
				...
				foreach( $data['stock_status'] as $language_id => $value )
				{
					...
					if( $stock_status_id > 0 )
					{
						$stmt = $db->prepare( 'INSERT INTO ' . TABLE_PRODUCT_NAME . '_stock_status SET 
							stock_status_id = ' . intval( $stock_status_id ) . ', 
					...
					else
					{
						...
						$stmt->bindParam( ':name', $value['name'], PDO::PARAM_STR );
						$stmt->execute();
						$stock_status_id = $db->lastInsertId();
						$stmt->closeCursor();
						unset( $stmt );
				...
				nv_insert_logs( NV_LANG_DATA, $module_name, 'Add A stock_status', 'stock_status_id: ' . $stock_status_id, $admin_info['userid'] );
			...
			{
				$db->query( 'DELETE FROM ' . TABLE_PRODUCT_NAME . '_stock_status WHERE stock_status_id = ' . ( int )$data['stock_status_id'] );
				...
				nv_insert_logs( NV_LANG_DATA, $module_name, 'Edit A stock_status', 'stock_status_id: ' . $data['stock_status_id'], $admin_info['userid'] );
	...
	$xtpl = new XTemplate( 'stock_status_add.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file . '/product' );
...
/*show list stock_status*/

$per_page = 50;
$page = $nv_Request->get_int( 'page', 'get', 1 );

$sql = TABLE_PRODUCT_NAME . '_stock_status
...
$base_url = NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=stock_status&amp;sort=' . $sort . '&amp;order=' . $order . '&amp;per_page=' . $per_page;
...
$xtpl = new XTemplate( 'stock_status.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file . '/product' );
$xtpl->assign( 'LANG', $lang_module );
$xtpl->assign( 'LANGE', $lang_ext );
$xtpl->assign( 'AddMenu', AddMenu() );
$xtpl->assign( 'NV_LANG_VARIABLE', NV_LANG_VARIABLE );
...
if( ! empty( $dataContent ) )
{
	foreach( $dataContent as $item )
	{
		...
		$item['token'] = md5( $global_config['sitekey'] . session_id() . $item['stock_status_id'] );

		$item['edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=stock_status&action=edit&token=' . $item['token'] . '&stock_status_id=' . $item['stock_status_id'];
