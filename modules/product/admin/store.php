...
if( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	if( $nv_Request->get_int( 'save', 'post', 0 ) == 1 )
	{
		...
		$config_store['config_customer_group_display'] = serialize( $config_store['config_customer_group_display'] );
		...
		$config_store['config_active_guest_order'] = $nv_Request->get_int( 'config_active_guest_order', 'post', 0 );
		...
		$config_store['config_processing_status'] = serialize( $config_store['config_processing_status'] );
		$config_store['config_complete_status'] = array_unique( $nv_Request->get_typed_array( 'config_complete_status', 'post', 'int', array() ) );
		$config_store['config_complete_status'] = serialize( $config_store['config_complete_status'] );
	...
	$config_store['config_customer_group_display'] = unserialize( $config_store['config_customer_group_display'] );
	$config_store['config_processing_status'] = unserialize( $config_store['config_processing_status'] );
	$config_store['config_complete_status'] = unserialize( $config_store['config_complete_status'] );
	...
	$check = ( $config_store['config_customer_price'] == '1' ) ? "checked=\"checked\"" : "";
	$xtpl->assign( 'ck_customer_price', $check );
	...
	$check = ( $config_store['config_active_guest_order'] == '1' ) ? "checked=\"checked\"" : "";
	...
	foreach( $productArrayYesNo as $stock_display => $values )
	{
		$checked = ( $stock_display == $config_store['config_stock_display'] ) ? 'checked="checked"' : '';
	...
	foreach( $productArrayYesNo as $stock_warning => $values )
	{
		$checked = ( $stock_warning == $config_store['config_stock_warning'] ) ? 'checked="checked"' : '';
	...
	foreach( $productArrayYesNo as $stock_checkout => $values )
	{
		$checked = ( $stock_checkout == $config_store['config_stock_checkout'] ) ? 'checked="checked"' : '';
	...
	foreach( $getOrderStatus as $processing_status => $value )
	{
		$value['checked'] = in_array( $processing_status, array_values( $config_store['config_processing_status'] ) ) ? " checked=\"checked\"" : "";
		$xtpl->assign( 'PSTATUS', $value );
		$xtpl->parse( 'main.processing_status' );
	}

	foreach( $getOrderStatus as $complete_status => $value )
	{
		$value['checked'] = in_array( $complete_status, array_values( $config_store['config_complete_status'] ) ) ? " checked=\"checked\"" : "";
