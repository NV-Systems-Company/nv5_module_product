...
if( ! function_exists( 'BlockMenuNavHorizontal' ) )
{

	/**
	 * ConfigBlockMenuNavHorizontal()
	 *
	 * @param mixed $module
	 ...
	 * @return
	 */
	function ConfigBlockMenuNavHorizontal( $module, $data_block, $lang_block )
	{
	...
	/**
	 * ConfigBlockMenuNavHorizontalSubmit()
	 *
	 * @param mixed $module
	 * @param mixed $lang_block
	 * @return
	 */
	function ConfigBlockMenuNavHorizontalSubmit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
	...
	/**
	 * BlockMenuNavHorizontal()
	 *
	 * @param mixed $block_config
	 * @return
	 */
	function BlockMenuNavHorizontal( $block_config )
	{
		...
		if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $mod_file . '/BlockMenuNavHorizontal.tpl' ) )
		{
		...
		$xtpl = new XTemplate( 'BlockMenuNavHorizontal.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file );
		...
		{ 
			...
			if( $cat['parent_id'] == 0 && $cat['inhome'] == '1')
			{
				...
				if( ! empty( $cat['subcatid'] ) )
				{
					...
					$xtpl->assign( 'SUBCAT', BlockMenuNavHorizontalSub( $cat['subcatid'], $title_length,  $block_theme ) );
	...
	function BlockMenuNavHorizontalSub( $list_sub, $title_length, $block_theme )
	{
		...
		{
			$xtpl = new XTemplate( 'BlockMenuNavHorizontal.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $productRegistry['mod_file'] );
...
{
	$content = BlockMenuNavHorizontal( $block_config );
}