...
if( $nv_Request->isset_request( 'save', 'get,post' ) )
{
	...
	if( ! $json )
	{
		...
		if( $nv_Request->get_title( 'payment_address', 'post', '', 1 ) == 'existing' )
		{
			...
			elseif( ! in_array( $address_id, array_keys( getAddresses() ) ) )
			{
				$json['error']['warning'] = $lang_ext['error_address'];
			}
			...
			if( ! $json )
			{
				...
				$_SESSION[$module_data . '_payment_address'] = getAddress( $address_id );
...
{
	$data['address_id'] = $_SESSION[$module_data . '_payment_address']['address_id'];
}
else
{
...
{
	$data['payment_address_custom_field'] = $_SESSION[$module_data . '_payment_address']['custom_field'];
}
else
{
...
$contents = checkout_payment_address( $data, $lang_ext );
echo $contents;
exit();
