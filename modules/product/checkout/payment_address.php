...
if( $nv_Request->isset_request( 'save', 'get,post' ) )
{
	...
	if( ! $json )
	{
		...
		$dataContent['payment_address'] = $nv_Request->get_string( 'payment_address', 'post', '' );
		
		if( $dataContent['payment_address']  == 'existing' )
		{
			if( empty( $dataContent['address_id'] ) )
			{
			...
			if( ! $json )
			{
				$_SESSION[$module_data . '_payment_address'] = getAddress( $globalUserid, $dataContent['address_id'] );
		...
		else
		{
			...
			$country_info = getCountries( $dataContent['country_id'] );
			...
			if( ! $json )
			{
				...
				$_SESSION[$module_data . '_payment_address'] = getAddress( $globalUserid, $address_id );
...
if( isset( $_SESSION[$module_data . '_payment_address']['address_id'] ) )
{
	$dataContent['address_id'] = $_SESSION[$module_data . '_payment_address']['address_id'];
}
else
{
...
$dataContent['addresses'] = getAddresses( $globalUserid );

if( isset( $_SESSION[$module_data . '_payment_address']['country_id'] ) )
{
	$dataContent['country_id'] = $_SESSION[$module_data . '_payment_address']['country_id'];
}
else
{
...
{
	$dataContent['zone_id'] = $_SESSION[$module_data . '_payment_address']['zone_id'];
}
else
{
	$dataContent['zone_id'] = '';
...
if( isset( $_SESSION[$module_data . '_payment_address']['custom_field'] ) )
{
	$dataContent['payment_address_custom_field'] = $_SESSION[$module_data . '_payment_address']['custom_field'];
}
else
{
	$dataContent['payment_address_custom_field'] = array();
}
 
$xtpl = new XTemplate( 'ThemeCheckoutPaymentAddress.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file . '/checkout' );
