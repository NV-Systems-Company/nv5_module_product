...
$bid = 1;
// block host
$num = $ProductGeneral->config['config_per_page'];
$data_content = array();
...
if( $ProductGeneral->config['config_active_price'] ) $xtpl->parse( 'form.price' );
...
$sql_i = ", if(t1.money_unit ='" . $ProductGeneral->config['config_money_unit'] . "', t1.product_price , t1.product_price * t2.exchange ) AS product_saleproduct ";
...
$show_price = "";
if( $ProductGeneral->config['config_active_price'] )
{
...
$table_exchange1 = " INNER JOIN " . $db_config['prefix'] . "_" . $module_data . "_category t3 ON t3.catid = t1.listcatid";
...
$db->select( "DISTINCT t1.id, t1.listcatid, t1.publtime, t1." . NV_LANG_DATA . "_title, t1." . NV_LANG_DATA . "_alias, t1." . NV_LANG_DATA . "_hometext, t1.homeimgalt, t1.image, t1.thumb, t1.product_number, t1.product_price, t1.discount_id, t1.money_unit, t1.showprice, t3.newday, t2.exchange " . $sql_i )
	->order( $order_by )
	->limit( $per_page )
...
while( list( $id, $listcatid, $publtime, $title, $alias, $hometext, $homeimgalt, $image, $thumb, $product_number, $product_price, $discount_id, $money_unit, $showprice, $newday ) = $result->fetch( 3 ) )
{
