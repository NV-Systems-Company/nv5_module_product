						...
						<table class="table table-bordered table-hover">
							<thead>
								<tr>
									...
									<td class="text-left"> <a href="{URL_GROUP}">{LANGE.column_attribute_group}</a> </td>
							...
							<tbody>
								 <!-- BEGIN: loop --> 
								<tr id="group_{LOOP.attribute_id}">
									<td class="text-center">
										<input type="checkbox" name="selected[]" value="{LOOP.attribute_id}"> 
									</td>
									<td class="text-left">{LOOP.name}</td>
									<td class="text-left">{LOOP.group_name}</td>
									...
									<td class="text-right">
										...
										&nbsp;&nbsp;
										<a href="javascript:void(0);" onclick="delete_attribute('{LOOP.attribute_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>
...
function delete_attribute(attribute_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=attribute&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
			data: 'attribute_id=' + attribute_id + '&token=' + token,
			beforeSend: function() {
