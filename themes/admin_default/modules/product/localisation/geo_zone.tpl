					...
					<table class="table table-bordered table-hover">
						<thead>
							<tr>
								...
								<td class="text-left"><a href="{URL_DESCRIPTION}">{LANGE.column_description}</a> </td>
								<td class="text-right"> <strong>{LANGE.column_action} </strong></td>
							</tr>
						</thead>
						<tbody>
							 <!-- BEGIN: loop --> 
							<tr id="group_{LOOP.geo_zone_id}">
								<td class="text-center">
									<input type="checkbox" name="selected[]" value="{LOOP.geo_zone_id}"> 
								</td>
								...
								<td class="text-right">
									...
									<a href="javascript:void(0);" onclick="delete_geo_zone('{LOOP.geo_zone_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>
								</td>
							</tr>
							<!-- END: loop -->
						</tbody>
...
function delete_geo_zone(geo_zone_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=geo_zone&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
			data: 'geo_zone_id=' + geo_zone_id + '&token=' + token,
			beforeSend: function() {
