			...
			<form action="#" method="post" enctype="multipart/form-data" id="form-weight-class">
				<div class="table-responsive">
					<table class="table table-bordered table-hover">
						...
						<tbody>
							 <!-- BEGIN: loop --> 
							<tr id="group_{LOOP.length_class_id}">
								<td class="text-center">
									<input type="checkbox" name="selected[]" value="{LOOP.length_class_id}"> 
								</td>
								...
								<td class="text-right">
									...
									<a href="javascript:void(0);" onclick="delete_length_class('{LOOP.length_class_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>
								</td>
							</tr>
							 <!-- END: loop -->
						</tbody>
...
function delete_length_class(length_class_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=length_class&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
			data: 'length_class_id=' + length_class_id + '&token=' + token,
			beforeSend: function() {
