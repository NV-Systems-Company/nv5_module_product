			...
			<form action="" method="post" enctype="multipart/form-data" id="form-coupon">
				<div class="table-responsive">
					<table class="table table-bordered table-hover">
						...
						<tbody>
							 <!-- BEGIN: loop --> 
							<tr id="group_{LOOP.tax_class_id}">
								<td class="text-center">
									<input type="checkbox" name="selected[]" value="{LOOP.tax_class_id}"> 
								</td>
								...
								<td class="text-right">
									...
									<a href="javascript:void(0);" onclick="delete_tax_class('{LOOP.tax_class_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>
								</td>
							</tr>
							 <!-- END: loop -->
						</tbody>
...
function delete_tax_class(tax_class_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=tax_class&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
			data: 'tax_class_id=' + tax_class_id + '&token=' + token,
			beforeSend: function() {
