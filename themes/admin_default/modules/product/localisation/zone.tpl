					...
					<table class="table table-bordered table-hover">
						<thead>
							<tr>
								...
								<td class="text-left"><a href="{URL_COUNTRY}">{LANGE.column_country}</a> </td>
								<td class="text-left"><a href="{URL_NAME}">{LANGE.column_name}</a> </td>
								<td class="text-center"><a href="{URL_CODE}">{LANGE.entry_code}</a> </td>
								<td class="text-center"><a href="{URL_STATUS}">{LANGE.entry_status}</a> </td>
								<td class="text-right"> <strong>{LANG.action} </strong></td>
							</tr>
						</thead>
						<tbody>
							 <!-- BEGIN: loop --> 
							<tr id="group_{LOOP.zone_id}">
								<td class="text-center">
									<input type="checkbox" name="selected[]" value="{LOOP.zone_id}"> 
								</td>
								<td class="text-left">{LOOP.country}</td>
								...
								<td class="text-right">
									...
									<a href="javascript:void(0);" onclick="delete_zone('{LOOP.zone_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>
								</td>
							</tr>
							 <!-- END: loop -->
						</tbody>
...
function delete_zone(zone_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=zone&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
			data: 'zone_id=' + zone_id + '&token=' + token,
			beforeSend: function() {
