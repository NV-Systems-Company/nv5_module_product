			...
			<form action="" method="post" enctype="multipart/form-data" id="form-coupon">
				<div class="table-responsive">
					<table class="table table-bordered table-hover">
						...
						<tbody>
							<!-- BEGIN: loop --> 
							<tr id="group_{LOOP.voucher_theme_id}">
								<td class="text-center">
									<input type="checkbox" name="selected[]" value="{LOOP.voucher_theme_id}"> 
								</td>
								...
								<td class="text-right">
									...
									<a href="javascript:void(0);" onclick="delete_voucher('{LOOP.voucher_theme_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>
								</td>
							</tr>
							 <!-- END: loop -->
						</tbody>
...
function delete_voucher(voucher_theme_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=voucher_theme&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
			data: 'voucher_theme_id=' + voucher_theme_id + '&token=' + token,
			beforeSend: function() {
