					...
					<table class="table table-bordered table-hover">
						<thead>
							<tr>
								...
								<td class="text-left"><strong><a href="{URL_NAME}">{LANGE.column_group}</a></strong></td>
								<td class="text-right"><strong><a href="{URL_SORT_ORDER}">{LANGE.column_sort_order}</a></strong></td>
								<td class="text-right"><strong>{LANGE.column_action}</strong></td>
							</tr>
						</thead>
						<tbody>
							 <!-- BEGIN: loop --> 
							<tr id="group_{LOOP.filter_group_id}">
								<td class="text-center">
									<input type="checkbox" name="selected[]" value="{LOOP.filter_group_id}"> 
								</td>
								...
								<td class="text-right">
									...
									<a href="javascript:void(0);" onclick="delete_filter_group('{LOOP.filter_group_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>
								</td>
							</tr>
							 <!-- END: loop -->
						</tbody>
...
function delete_filter_group(filter_group_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=filter&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
			data: 'filter_group_id=' + filter_group_id + '&token=' + token,
			beforeSend: function() {
