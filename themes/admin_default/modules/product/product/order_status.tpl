			...
			<form action="#" method="post" enctype="multipart/form-data" id="form-block">
				<div class="table-responsive">
					<table class="table table-bordered table-hover">
						...
						<tbody>
							 <!-- BEGIN: loop --> 
							<tr id="group_{LOOP.order_status_id}">
								<td class="text-center">
									<input type="checkbox" name="selected[]" value="{LOOP.order_status_id}"> 
								</td>	 
								...
								<td class="text-right">
									...
									<a href="javascript:void(0);" onclick="delete_order_status('{LOOP.order_status_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>
								</td>
							</tr>
							 <!-- END: loop -->
						</tbody>
...
function delete_order_status(order_status_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=order_status&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
			data: 'order_status_id=' + order_status_id + '&token=' + token,
			beforeSend: function() {
